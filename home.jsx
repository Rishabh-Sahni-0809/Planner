...
export default function CompanyPlanner() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const r = await window.storage.get("company_planner_v3");
        if (r?.value) setDone(JSON.parse(r.value));
      } catch {}
    })();
  }, []);
  ...
  const toggleTask = async (id) => {
    ...
    setDone(n);
    try { await window.storage.set("company_planner_v3", JSON.stringify(n)); } catch {}
  };
